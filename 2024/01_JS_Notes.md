...
* So:
  ...
  * 100ms = 0.1 sec (fastest)
  * 300ms = 0.3 sec
  * 500ms = 0.5 sec
  * 1000ms = 1 sec

👉 Smaller ms = faster execution 👍
...
Promises are used to handle asynchronous operations more effectively by avoiding callback hell, improving readability, and providing better error handling through chaining and catch blocks



 A Promise has three states: pending, fulfilled, and rejected.  

1. Pending → The initial state; neither fulfilled nor rejected yet.
2. Fulfilled → The operation completed successfully.
3. Rejected → The operation failed.

* Settled is not a state.
  It means the promise has finished execution and is either fulfilled or rejected.
...
p and returns a single promise that resolves with an array of results when all promises are fulfilled.
...
Using `new` is invalid:

```javascript
new Promise.resolve(true); // ❌ Error
```

### `new Promise()`

Use it when you want to control when the promise resolves or rejects.
...
`new Promise()` is used to create and control asynchronous operations, while `Promise.resolve()` and `Promise.reject()` are static methods that create already settled promises without using the `new` keyword.
...
Promise chaining is used to perform multiple asynchronous operations sequentially while improving code readability, avoiding callback hell, and Provides centralized error handling using .catch().
...
- Cleaner
- Easier to read
- Looks synchronous but is fully asynchronous
...
- One operation depends on the result of the previous operation.
- You want cleaner and more readable code.
- You want simpler error handling using `try...catch`.


### Use Promises when:

- You need to handle multiple asynchronous operations in parallel.
- You want to use methods like `Promise.all()`, `Promise.race()`, `Promise.any()`, or `Promise.allSettled()`.
- You need fine-grained control over promise chaining.
- You are working with APIs that already return promises.


### 🧠 Interview one-liner:

Use `async/await` for sequential asynchronous operations and better readability, and use Promise methods when handling multiple asynchronous operations in parallel.



 Use `async/await` for readability, but combine it with `Promise.all()` for parallelism when needed.
```js
