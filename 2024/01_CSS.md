...
- `The CSS Box Model` is a fundamental concept that describes how HTML elements are postioned , sized and displayed on a web page. 
...
allowing developers to write CSS with additional features such as variables, nesting, functions, mixins, and loops code.
...
- The `@media rule` is used to define media queries to apply different styles for different media types/devices.
...
/* Outside .container, --main-color is not available */
.outside {
  color: var(--main-color);
}
```
